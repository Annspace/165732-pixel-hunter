...
export const gameThree = (gameData) => {
  const content = `<div class="game">
    ...
    <form class="game__content  game__content--triple">
      <div class="game__option">
        <img src="${gameData[state.screenIndex].question.src1}" alt="Option 1" width="304" height="455">
      </div>
      <div class="game__option  game__option--selected">
        <img src="${gameData[state.screenIndex].question.src2}" alt="Option 1" width="304" height="455">
      </div>
      <div class="game__option">
        <img src="${gameData[state.screenIndex].question.src3}" alt="Option 1" width="304" height="455">
      </div>
    </form>
    <div class="stats">
       ${stats(state.answers)}
  ...
  for (let option of gameOptions) {
    option.addEventListener(`click`, (e) => {
      ...
      state.screenIndex++;
      if (state.screenIndex < gameData.length) {
        changeScreen(gameThree(gameData));
      } else {
        state.result.gameThree.answers = state.answers.slice(6, gameData.length + 1);
        state.result.gameThree.lives = state.lives;
